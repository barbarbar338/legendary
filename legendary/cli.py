...
class LegendaryCLI:
    ...
    def auth(self, args):
        ...
        if args.import_egs_auth:
            ...
            if not self.core.egl.appdata_path:
                ...
                if os.path.exists(lutris_wine_pfx):
                    ...
                    if args.yes or get_boolean_choice(
                        "Do you want to use the Lutris install?"
                    ):
                        egl_wine_pfx = lutris_wine_pfx

                if not egl_wine_pfx:
                    logger.info(
                        "Please enter the path to the Wine prefix that has EGL installed"
                    )
                ...
                try:
                    wine_folders = get_shell_folders(
                        read_registry(egl_wine_pfx), egl_wine_pfx
                    )
                    egl_appdata = os.path.realpath(
                        os.path.join(
                            wine_folders["Local AppData"],
                            "EpicGamesLauncher",
                            "Saved",
                            "Config",
                            "Windows",
                    ...
                    logger.error(
                        f"Got exception when trying to read WINE registry: {e!r}"
                    )
                ...
                if not os.path.exists(egl_appdata):
                    logger.error(
                        f'Wine prefix does not have EGL appdata path at "{egl_appdata}"'
                    )
                    exit(0)
                else:
            ...
            try:
                ...
                logger.error(
                    f"No EGS login session found, please login manually. (Exception: {e!r})"
                )
                exit(1)
        ...
        if not args.auth_code and not args.session_id and not args.ex_token:
            ...
            if not webview_available or args.no_webview or self.core.webview_killswitch:
                ...
                webbrowser.open(url)
                print(
                    f"If the web page did not open automatically, please manually open the following URL: {url}"
                )
                auth_code = input(
                ...
                if do_webview_login(
                    callback_code=self.core.auth_ex_token,
                    user_agent=f"EpicGamesLauncher/{self.core.get_egl_version()}",
                ):
                    logger.info(
                        f'Successfully logged in as "{self.core.lgd.userdata["displayName"]}" via WebView'
                    )
                else:
                    logger.error(
                        "WebView login attempt failed, please see log for details."
                    )
                return
    ...
    def list_games(self, args):
        ...
        if args.include_noasset:
            na_games, na_dlcs = self.core.get_non_asset_library_items(
                skip_ue=not args.include_ue
            )
        ...
        for citem_id in dlc_list.keys():
            ...
            dlc_list[citem_id] = sorted(
                dlc_list[citem_id], key=lambda d: d.app_title.lower()
            )
        ...
        for game in games:
            ...
            print(
                f" * {game.app_title.strip()} (App name: {game.app_name} | Version: {version})"
            )
            ...
            if not version:
                ...
                if _store in ["Origin", "the EA app"]:
                    print(
                        f"  - This game has to be activated, installed, and launched via Origin/EA app, use "
                        f'"legendary launch --origin {game.app_name}" to activate and/or run the game.'
                    )
                elif _store:
                    print(
                        f"  ! This game has to be installed through a third-party store ({_store}, not supported)"
                    )
                else:
            ...
            if game.partner_link_type:
                ...
                if _type == "ubisoft":
                    ...
                    print(
                        f'  ! This app requires linking to a third-party account (name: "{_type}", not supported)'
                    )

            for dlc in dlc_list[game.catalog_item_id]:
                print(
                    f"  + {dlc.app_title} (App name: {dlc.app_name} | Version: {dlc.app_version(args.platform)})"
                )
                if not dlc.app_version(args.platform):
                    print(
                        f"   ! This DLC is either included in the base game, or not available for {args.platform}"
                    )
    ...
    def list_installed(self, args):
        ...
        games = sorted(
            self.core.get_installed_list(include_dlc=True),
        ...
        for game in games:
            try:
                versions[game.app_name] = self.core.get_asset(
                    game.app_name, platform=game.platform
                ).build_version
            except ValueError:
                logger.warning(
                    f'Metadata for "{game.app_name}" is missing, the game may have been removed from '
        ...
        if args.csv or args.tsv:
            ...
            writer.writerows(
                (
                    ...
                    game.title,
                    game.version,
                    versions[game.app_name],
                    versions[game.app_name] != game.version,
                    game.install_size,
                    game.install_path,
                    game.platform,
                )
                for game in games
                if game.app_name in versions
            )
            return
        ...
        for game in games.copy():
            ...
            if not dlc or not dlc.metadata:
                logger.warning(
                    f'DLC "{game.app_name}" is missing metadata for some reason. '
        ...
        for game in games:
            if game.install_size == 0 and self.core.lgd.lock_installed():
                ...
                m = self.core.load_manifest(
                    self.core.get_installed_manifest(game.app_name)[0]
                )
                game.install_size = sum(
                    fm.file_size for fm in m.file_manifest_list.elements
                )
            ...
            if not os.path.exists(game.install_path):
                print(
                    f'  ! Game does no longer appear to be installed (directory "{game.install_path}" missing)!'
                )
            elif game.app_name in versions and versions[game.app_name] != game.version:
                print(
                    f"  -> Update available! Installed: {game.version}, Latest: {versions[game.app_name]}"
                )
            for dlc in installed_dlcs[game.app_name]:
                print(
                    f"  + {dlc.title} (App name: {dlc.app_name} | Version: {dlc.version}) | "
                    f"{dlc.install_size / (1024 * 1024 * 1024):.02f} GiB)"
                )
                if dlc.app_name in versions and versions[dlc.app_name] != dlc.version:
                    print(
                        f"   -> Update available! Installed: {dlc.version}, Latest: {versions[dlc.app_name]}"
                    )
    ...
    def list_files(self, args):
        ...
        if args.override_manifest:
            ...
            if not game:
                logger.fatal(
                    f'Could not fetch metadata for "{args.app_name}" (check spelling/account ownership)'
                )
                exit(1)
        ...
        if args.hashlist:
            ...
            writer.writerows(
                (fm.filename, fm.hash.hex(), fm.file_size, "|".join(fm.install_tags))
                for fm in files
            )
        elif args.json:
            _files = [
    ...
    def clean_saves(self, args):
        ...
        self.core.clean_saves(
            self._resolve_aliases(args.app_name), args.delete_incomplete
        )
        ...
        for igame in igames:
            ...
            if not game or not (
                ...
                if igame.app_name in latest_save:
                    ...
                    logger.warning(
                        f"{igame.app_name} has remote save(s) but does not support cloud saves?!"
                    )
                continue
            ...
            if args.app_name and args.save_path:
                if not self.core.lgd.lock_installed():
                    logger.error(
                        "Unable to lock install data, cannot modify save path."
                    )
                    break
            ...
            if not igame.save_path and self.core.lgd.lock_installed():
                if args.yes and not args.accept_path:
                    logger.info(
                        "Save path for this title has not been set, skipping due to --yes"
                    )
                    continue

                save_path = self.core.get_save_path(
                    igame.app_name, platform=igame.platform
                )
                ...
                if not yn:
                    save_path = input(
                        "Please enter the correct path (leave empty to skip): "
                    )
                    if not save_path:
            ...
            res, (dt_l, dt_r) = self.core.check_savegame_state(
                igame.save_path, latest_save.get(igame.app_name)
            )
            ...
            if res == SaveGameStatus.SAME_AGE and not (
                args.force_upload or args.force_download
            ):
                logger.info(f'Save game for "{igame.title}" is up to date, skipping...')
                continue

            if (
                res == SaveGameStatus.REMOTE_NEWER and not args.force_upload
            ) or args.force_download:
                if (
                    res == SaveGameStatus.REMOTE_NEWER
                ):  # only print this info if not forced
                    logger.info(f'Cloud save for "{igame.title}" is newer:')
                    logger.info(
                        f'- Cloud save date: {dt_r.strftime("%Y-%m-%d %H:%M:%S")}'
                    )
                    if dt_l:
                        logger.info(
                            f'- Local save date: {dt_l.strftime("%Y-%m-%d %H:%M:%S")}'
                        )
                    else:
                ...
                self.core.upload_save(
                    igame.app_name, igame.save_path, dt_l, args.disable_filters
                )
    ...
    def launch_game(self, args, extra):
        ...
        if (not igame or not igame.executable) and (
        ...
        if igame.is_dlc and not igame.executable:
            logger.error(
                f"{app_name} is DLC without an executable; please launch the base game instead!"
            )
            exit(1)

        if not os.path.exists(igame.install_path):
            logger.fatal(
                f'Install directory "{igame.install_path}" appears to be deleted, cannot launch {app_name}!'
            )
            exit(1)
            ...
            if not args.skip_version_check and not self.core.is_noupdate_game(app_name):
                ...
                try:
                    latest = self.core.get_asset(
                        app_name, update=True, platform=igame.platform
                    )
                except ValueError:
                    logger.fatal(
                        f'Metadata for "{app_name}" does not exist, cannot launch!'
                    )
                    exit(1)
        ...
        params = self.core.get_launch_parameters(
            app_name=app_name,
            offline=args.offline,
            extra_args=extra,
            user=args.user_name_override,
            wine_bin=args.wine_bin,
            wine_pfx=args.wine_pfx,
        ...
        if "CX_BOTTLE" in full_env and any(
            "SharedSupport/CrossOver" in p for p in params.launch_command
        ):
            ...
            if not mac_is_valid_bottle(bottle_name):
                if bottle_name == "Legendary":
                    logger.error(
                        'Attempted to use default CrossOver bottle ("Legendary"), but it does not exist, '
                        "see https://legendary.gl/crossover-setup for setup instructions."
                    )
                else:
                    logger.error(
                        f"Specified CrossOver bottle {bottle_name} does not exist, cannot launch."
                    )
                exit(1)
            else:
        ...
        if args.dry_run:
            ...
            if params.environment:
                logger.info(
                    "Environment overrides: {}".format(
                        ", ".join(f"{k}={v}" for k, v in params.environment.items())
                    )
                )
        else:
            ...
            if params.pre_launch_command:
                try:
                    logger.debug(
                        f"Running pre-launch command: {params.pre_launch_command}"
                    )
                    p = subprocess.Popen(
                        shlex.split(params.pre_launch_command), env=full_env
                    )
    ...
    def _launch_origin(self, args):
        ...
        if not game:
            logger.error(
                f'Unknown game "{args.app_name}", run "legendary list-games --third-party" '
                f"to fetch data for Origin titles before using this command."
            )
            return

        if not game.third_party_store or game.third_party_store not in [
            "Origin",
            "the EA app",
        ]:
        ...
        command = self.core.get_app_launch_command(
            args.app_name,
            wrapper=args.wrapper,
        ...
        env = self.core.get_app_environment(
            args.app_name, wine_pfx=args.wine_pfx, cx_bottle=args.crossover_bottle
        )
        cmd, wait_for_exit = self.core.get_pre_launch_command(args.app_name)
        ...
        if "CX_BOTTLE" in full_env and any(
            "SharedSupport/CrossOver" in p for p in command
        ):
            # if using crossover, unset WINEPREFIX
            full_env.pop("WINEPREFIX", None)
            # check that bottle is valid, show error otherwise
        ...
        if not command:
            logger.error(
                f"In order to launch Origin correctly you must specify a prefix and wine binary or "
    ...
    def install_game(self, args):
        if not self.core.lgd.lock_installed():
            logger.fatal(
                "Failed to acquire installed data lock, only one instance of Legendary may "
        ...
        if self.core.is_installed(args.app_name):
            ...
            if igame.needs_verification and not args.repair_mode:
                logger.info(
                    "Game needs to be verified before updating, switching to repair mode..."
                )
        ...
        if args.subparser_name == "download":
            ...
            repair_file = os.path.join(
                self.core.lgd.get_tmp_path(), f"{args.app_name}.repair"
            )
        ...
        game = self.core.get_game(
            args.app_name, update_meta=True, platform=args.platform
        )
        ...
        if store := game.third_party_store:
            logger.error(
                f"The selected title has to be installed via a third-party store: {store}"
            )
            if store in ["Origin", "the EA app"]:
                logger.info(
                    f'For Origin games use "legendary launch --origin {args.app_name}" to '
        ...
        if args.platform not in game.asset_infos:
            if not args.no_install:
                if self.core.lgd.config.getboolean(
                    "Legendary", "install_platform_fallback", fallback=True
                ):
                    logger.warning(
                        f'App has no asset for platform "{args.platform}", falling back to "Windows".'
                    )
                    args.platform = "Windows"
                else:
                    logger.error(
                        f'No app asset found for platform "{args.platform}", run '
                ...
                logger.warning(
                    f'No asset found for platform "{args.platform}", '
        ...
        if args.repair_mode:
            if not self.core.is_installed(game.app_name):
                logger.error(
                    f'Game "{game.app_title}" ({game.app_name}) is not installed!'
                )
                exit(0)
            ...
            if not os.path.exists(repair_file):
                ...
                if not args.yes:
                    if not get_boolean_choice(
                        f'Verify "{game.app_name}" now ("no" will abort repair)?'
                    ):
                        print("Aborting...")
                        exit(0)
                ...
                try:
                    self.verify_game(
                        ...
                        print_command=False,
                        repair_mode=True,
                        repair_online=args.repair_and_update,
                    )
                except ValueError:
                    logger.error(
                        "To repair a game with a missing manifest you must run the command with "
        ...
        config_tags = self.core.lgd.config.get(
            game.app_name, "install_tags", fallback=None
        )
        config_disable_sdl = self.core.lgd.config.getboolean(
            game.app_name, "disable_sdl", fallback=False
        )
        ...
        # if config flag is not yet set, set it and remove previous install tags
            ...
            logger.info(
                "Clearing install tags from config and disabling SDL for title."
            )
            if config_tags:
        ...
        if sdl_enabled and ((sdl_name := get_sdl_appname(game.app_name)) is not None):
            if (
                ...
                if sdl_data:
                    ...
                    self.core.lgd.config.set(
                        game.app_name, "install_tags", ",".join(args.install_tag)
                    )
                else:
            ...
            logger.info(
                f'Saving install tags for "{game.app_name}" to config: {config_tags}'
            )
        ...
        if not analysis.dl_size:
            ...
            if old_igame.install_tags:
                self.core.lgd.config.set(
                    game.app_name, "install_tags", ",".join(old_igame.install_tags)
                )
            ...
            if not args.no_install:
                ...
                if dlcs and not args.skip_dlcs:
                    ...
                    if not args.yes and not args.with_dlcs and not args.skip_dlcs:
                        if not get_boolean_choice(
                            f"Do you wish to automatically install DLCs?"
                        ):
                            install_dlcs = False
                ...
                if (
                    ...
                    logger.info(
                        f'This game supports cloud saves, syncing is handled by the "sync-saves" command. '
    ...
    def _handle_postinstall(self, postinstall, igame, skip_prereqs=False):
        ...
        if os.name == "nt":
            ...
            if c == "i":  # just set it to installed
                ...
                try:
                    p = subprocess.Popen(
                        [fullpath, postinstall["args"]], cwd=work_dir, shell=True
                    )
                    p.wait()
                except Exception as e:
    ...
    def _handle_uninstaller(self, igame, yes=False):
        ...
        if yes or get_boolean_choice(
            "Do you wish to run the uninstaller?", default=True
        ):
            ...
            try:
                p = subprocess.Popen(
                    [fullpath, uninstaller["args"]], cwd=work_dir, shell=True
                )
                p.wait()
            except Exception as e:
    ...
    def verify_game(
        self, args, print_command=True, repair_mode=False, repair_online=False
    ):
        args.app_name = self._resolve_aliases(args.app_name)
        if not self.core.is_installed(args.app_name):
        ...
        if not os.path.exists(igame.install_path):
            logger.error(
                f'Install path "{igame.install_path}" does not exist, make sure all necessary mounts '
        ...
        if manifest_data is None:
            if repair_mode:
                if not repair_online:
                    logger.critical(
                        "No manifest could be loaded, the manifest file may be missing!"
                    )
                    raise ValueError("Local manifest is missing")

                logger.warning(
                    "No manifest could be loaded, the file may be missing. Downloading the latest manifest."
                )
                ...
                logger.critical(
                    f'Manifest appears to be missing! To repair, run "legendary repair '
        ...
        if (
            config_tags := self.core.lgd.config.get(
                args.app_name, "install_tags", fallback=None
            )
        ) is not None:
            ...
            file_list = [
                ...
                for f in files
                if any(it in install_tags for it in f.install_tags)
                or not f.install_tags
            ]
        else:
        ...
        total = len(file_list)
        total_size = sum(
            manifest.file_manifest_list.get_file_by_path(fm[0]).file_size
            for fm in file_list
        )
        ...
        repair_file = []
        for result, path, result_hash, bytes_read in validate_files(
            ...
            stdout.write(
                f"Verification progress: {num}/{total} ({percentage:.01f}%) [{speed:.1f} MiB/s]\t\r"
            )
            stdout.flush()
        ...
        stdout.write(
            f"Verification progress: {num}/{total} ({percentage:.01f}%) [{speed:.1f} MiB/s]\t\n"
        )
        ...
        if not missing and not failed:
            ...
            if print_command:
                logger.info(
                    f'Run "legendary repair {args.app_name}" to repair your game installation.'
                )
    ...
    def import_game(self, args):
        ...
        if game.is_dlc:
            ...
            if release_info:
                ...
                if not self.core.is_installed(main_game_appname):
                    logger.error(
                        f'Import candidate is DLC but base game "{main_game_title}" '
                        f'(App name: "{main_game_appname}") is not installed!'
                    )
                    return
            else:
                logger.fatal(
                    f"Unable to get base game information for DLC, cannot continue."
                )
                return

        # get everything needed for import from core, then run additional checks.
        manifest, igame = self.core.import_game(
            game, args.app_path, platform=args.platform
        )
        ...
        found = sum(
            os.path.exists(os.path.join(args.app_path, f.filename))
        ...
        if not found:
            logger.error(
                f'No files belonging to {"DLC" if game.is_dlc else "Game"} "{game.app_title}" '
        ...
        if not game.is_dlc and not os.path.exists(exe_path) and not args.disable_check:
            logger.error(
                f'Game executable could not be found at "{exe_path}", '
        ...
        if ratio < 0.95:
            ...
            logger.info(
                f'{"DLC" if game.is_dlc else "Game"} install appears to be complete.'
            )

        self.core.install_game(igame)
        if igame.needs_verification:
            logger.info(
                f'NOTE: The {"DLC" if game.is_dlc else "Game"} installation will have to be '
                f"verified before it can be updated with legendary."
            )
            logger.info(f'Run "legendary repair {args.app_name}" to do so.')
        else:
            logger.info(
                f'Installation had Epic Games Launcher metadata for version "{igame.version}", '
        ...
        if not args.skip_dlcs:
            ...
            if dlcs:
                ...
                if not args.yes and not args.with_dlcs:
                    if not get_boolean_choice(
                        f"Do you wish to automatically attempt to import all DLCs?"
                    ):
                        import_dlc = False
        ...
        logger.info(
            f'{"DLC" if game.is_dlc else "Game"} "{game.app_title}" has been imported.'
        )
    ...
    def egs_sync(self, args):
        ...
        if args.unlink:
            ...
            logger.info(
                "NOTE: All games are still available in Legendary and EGL, but future changes "
        ...
        if not self.core.lgd.assets:
            logger.error(
                "Legendary is missing game metadata, please login (if not already) and use the "
        ...
        if args.migrate:
            logger.info(
                "Migration enabled, this will remove the games from EGL, but not uninstall them."
            )
            ...
            if not args.egl_manifest_path and not args.egl_wine_prefix:
                ...
                if os.path.exists(lutris_data_path):
                    ...
                    if args.yes or get_boolean_choice(
                        ...
                        if not os.path.exists(egl_path):
                            print(
                                "EGL Data path exists but Manifests directory is missing, creating..."
                            )
                ...
                if not egl_path:
                    ...
                    egl_data_path = os.path.join(
                        egl_path, "drive_c/ProgramData/Epic/EpicGamesLauncher/Data"
                    )
                    egl_path = os.path.join(egl_data_path, "Manifests")
                    if not os.path.exists(egl_path):
                        if not os.path.exists(egl_data_path):
                            print(
                                "Invalid path (wrong directory, WINE prefix, or EGL not installed/launched)"
                            )
                            exit(1)
                        print(
                            "EGL Data path exists but Manifests directory is missing, creating..."
                ...
                if not os.listdir(egl_path):
                    logger.warning(
                        "Folder is empty, this may be fine if nothing has been installed yet."
                    )
                ...
                if not os.path.exists(args.egl_manifest_path):
                    logger.fatal(
                        "Path specified via --egl-manifest-path does not exist"
                    )
                    exit(1)
                self.core.egl.programdata_path = args.egl_manifest_path
                self.core.lgd.config.set(
                    "Legendary", "egl_programdata", args.egl_manifest_path
                )

        logger.debug(
            f'Using EGL ProgramData path "{self.core.egl.programdata_path}"...'
        )
        ...
        if not args.export_only:
            ...
            if importable:
                ...
                if args.yes or get_boolean_choice(
                    "Do you want to import the games from EGL?"
                ):
                    for egl_game in importable:
        ...
        if not args.import_only:
            ...
            if exportable:
                ...
                if args.yes or get_boolean_choice(
                    "Do you want to export the games to EGL?"
                ):
                    for lgd_game in exportable:
        ...
        if not self.core.egl_sync_enabled and not args.one_shot and not args.migrate:
            if not args.enable_sync:
                args.enable_sync = args.yes or get_boolean_choice(
            ...
            logger.info(
                "Disabling automatic sync (if enabled) and removing EGL link to finish migration..."
            )
            self.core.lgd.config.remove_option("Legendary", "egl_programdata")
            self.core.lgd.config.remove_option("Legendary", "egl_sync")
            ...
            logger.info(
                "Migration complete. Your games will now be exclusively managed by Legendary."
            )
        else:
            self.core.egl_sync()
    ...
    def status(self, args):
        ...
        if self.core.update_available:
            if update_info := self.core.get_update_info():
                print(
                    f'- New version: {update_info["version"]} - "{update_info["name"]}"'
                )
                print(
    ...
    def info(self, args):
        ...
        if self.core.is_installed(app_name):
            ...
            if installed_platform != args.platform:
                logger.warning(
                    f'Game is installed for platform "{installed_platform}", '
                    f'but requested metadata is for "{args.platform}", this may '
                    f"lead to unexpected results."
                )

        game = self.core.get_game(
            app_name, update_meta=not args.offline, platform=args.platform
        )
        ...
        if args.offline or manifest_uri:
            if app_name and self.core.is_installed(app_name):
                ...
                logger.info(
                    "Game not installed and offline mode enabled, cannot load manifest."
                )
        elif game:
        ...
        if game:
            ...
            game_infos.append(
                InfoItem("App name", "app_name", game.app_name, game.app_name)
            )
            game_infos.append(
                InfoItem("Title", "title", game.app_title, game.app_title)
            )
            game_infos.append(
                InfoItem(
                    "Latest version",
            ...
            game_infos.append(
                InfoItem(
                    "All versions", "platform_versions", all_versions, all_versions
                )
            )
            ...
            game_infos.append(
                InfoItem(
                    "Cloud save folder (Windows)", "cloud_save_folder", cs_dir, cs_dir
                )
            )

            cs_dir = None
            if game.supports_mac_cloud_saves:
                cs_dir = game.metadata["customAttributes"]["CloudSaveFolder_MAC"][
                    "value"
                ]
            game_infos.append(
                InfoItem(
                    "Cloud save folder (Mac)", "cloud_save_folder_mac", cs_dir, cs_dir
                )
            )
            ...
            game_infos.append(
                InfoItem(
                    "Activates on external platform",
                    "external_activation",
                    external_activation or "No",
                    external_activation,
                )
            )
            ...
            launch_options = []
            i = 1
            while f"extraLaunchOption_{i:03d}_Name" in game.metadata.get(
                "customAttributes", {}
            ):
                launch_options.append(
                    (
                        game.metadata["customAttributes"][
                            f"extraLaunchOption_{i:03d}_Name"
                        ]["value"],
                        game.metadata["customAttributes"][
                            f"extraLaunchOption_{i:03d}_Args"
                        ]["value"],
                    )
                )
                i += 1
            ...
            if launch_options:
                ...
                game_infos.append(
                    InfoItem(
                        "Extra launch options", "launch_options", human_list, json_list
                    )
                )
            else:
                game_infos.append(
                    InfoItem("Extra launch options", "launch_options", None, [])
                )
            ...
            if entitlements and not game.is_dlc:
                ...
                owned_app_names = {
                    g.app_name for g in self.core.get_assets(args.platform)
                }
                owned_dlc = []
                for dlc in game.metadata.get("dlcItemList", []):
                    ...
                    if dlc["entitlementName"] in owned_entitlements:
                        ...
                        if dlc_app_name in owned_app_names:
                            owned_dlc.append(
                                (installable, dlc_app_name, dlc["title"], dlc["id"])
                            )
                ...
                if owned_dlc:
                    ...
                    for installable, dlc_app_name, title, dlc_id in owned_dlc:
                        ...
                        if installable:
                            human_list.append(
                                f'App name: {dlc_app_name}, Title: "{title}"'
                            )
                        else:
                            human_list.append(
                                f'Title: "{title}" (no installation required)'
                            )
                    game_infos.append(
                        InfoItem("Owned DLC", "owned_dlc", human_list, json_list)
                    )
                else:
            ...
            if igame:
                ...
                installation_info.append(
                    InfoItem("Platform", "platform", igame.platform, igame.platform)
                )
                installation_info.append(
                    InfoItem("Version", "version", igame.version, igame.version)
                )
                disk_size_human = f"{igame.install_size / 1024 / 1024 / 1024:.02f} GiB"
                installation_info.append(
                    InfoItem(
                        "Install size", "disk_size", disk_size_human, igame.install_size
                    )
                )
                installation_info.append(
                    InfoItem(
                ...
                installation_info.append(
                    InfoItem(
                        "Save data path", "save_path", igame.save_path, igame.save_path
                    )
                )
                installation_info.append(
                    InfoItem(
                ...
                installation_info.append(
                    InfoItem("Install tags", "install_tags", tags, igame.install_tags)
                )
                installation_info.append(
                    InfoItem(
                        "Requires ownership verification token (DRM)",
                        "requires_ovt",
                        igame.requires_ot,
                        igame.requires_ot,
                    )
                ...
                for dlc in game.metadata.get("dlcItemList", []):
                    ...
                    if igame := self.core.get_installed_game(app_name):
                        ...
                        installed_dlc_human.append(
                            'App name: {}, Title: "{}", Size: {:.02f} GiB'.format(
                                igame.app_name,
                                igame.title,
                                igame.install_size / 1024 / 1024 / 1024,
                            )
                        )
                installation_info.append(
                    InfoItem(
        ...
        if manifest_data:
            ...
            manifest_info.append(
                InfoItem("Manifest size", "size", manifest_size_human, manifest_size)
            )
            manifest_type = "JSON" if hasattr(manifest, "json_data") else "Binary"
            manifest_info.append(
                InfoItem("Manifest type", "type", manifest_type, manifest_type.lower())
            )
            manifest_info.append(
                InfoItem(
                    "Manifest version", "version", manifest.version, manifest.version
                )
            )
            manifest_info.append(
                InfoItem(
                    "Manifest feature level",
                    "feature_level",
                    manifest.meta.feature_level,
                    manifest.meta.feature_level,
                )
            )
            manifest_info.append(
            ...
            if manifest.meta.prereq_ids:
                ...
                manifest_info.append(
                    InfoItem("Prerequisites", "prerequisites", None, None)
                )
            ...
            if manifest.meta.uninstall_action_path:
                ...
                manifest_info.append(
                    InfoItem(
                        ...
                        human_list,
                        dict(
                            path=manifest.meta.uninstall_action_path,
            ...
            manifest_info.append(
                InfoItem(
                    "Install tags", "install_tags", install_tags_human, install_tags
                )
            )
            ...
            manifest_info.append(
                InfoItem("Disk size (uncompressed)", "disk_size", file_size, total_size)
            )
            # total chunk size
            ...
            if len(install_tags) > 1:
                ...
                for tag in install_tags:
                    ...
                    tag_files = [
                        fm
                        for fm in manifest.file_manifest_list.elements
                        if (tag in fm.install_tags) or (not tag and not fm.install_tags)
                    ]
                    tag_file_size = sum(fm.file_size for fm in tag_files)
                    tag_disk_size.append(
                        dict(tag=tag, size=tag_file_size, count=len(tag_files))
                    )
                    tag_file_size_human = "{:.02f} GiB".format(
                        tag_file_size / 1024 / 1024 / 1024
                    )
                    tag_disk_size_human.append(
                        f"{human_tag.ljust(longest_tag)} - {tag_file_size_human} "
                    ...
                    tag_download_size.append(
                        dict(tag=tag, size=tag_chunk_size, count=len(tag_chunk_guids))
                    )
                    tag_chunk_size_human = "{:.02f} GiB".format(
                        tag_chunk_size / 1024 / 1024 / 1024
                    )
            ...
            manifest_info.append(
                InfoItem(
                    "Disk size by install tag",
                    "tag_disk_size",
                    tag_disk_size_human or "N/A",
                    tag_disk_size,
                )
            )
            manifest_info.append(
                InfoItem(
                    "Download size by install tag",
                    "tag_download_size",
                    tag_download_size_human or "N/A",
                    tag_download_size,
                )
            )
    ...
    def alias(self, args):
        if args.action == "add":
            ...
            logger.info(
                f'Added alias "{alias}" to "{app_name}" (Title: "{game.app_title}")'
            )
        elif args.action == "rename":
            old_alias = args.app_or_alias
            new_alias = args.alias
            app_name = self.core.lgd.config.get(
                "Legendary.aliases", old_alias, fallback=None
            )
            if not app_name:
            ...
            if args.app_or_alias:
                ...
                if self.core.lgd.config.has_section("Legendary.aliases"):
                    aliases = [
                        alias
                        for (alias, app_name) in self.core.lgd.config[
                            "Legendary.aliases"
                        ].items()
                ...
                aliases = [
                    alias
                    for (alias, app_name) in self.core.lgd.aliases.items()
                    if app_name == game.app_name
                ]
                if aliases:
    ...
    def cleanup(self, args):
        ...
        for _platform in self.core.get_installed_platforms():
            app_names |= set(
                g.app_name
                for g in self.core.get_assets(update_assets=False, platform=_platform)
            )
        ...
        if not args.keep_manifests:
            ...
            installed = [
                (ig.app_name, ig.version, ig.platform)
                for ig in self.core.get_installed_list()
            ]
            installed.extend(
                (ig.app_name, ig.version, ig.platform)
                for ig in self.core.get_installed_dlc_list()
            )
    ...
    def activate(self, args):
        ...
        if args.uplay:
            ...
            for ext_auth in ext_auths:
                ...
                print(
                    "If the web page did not open automatically, please manually open the following URL: "
            ...
            if not uplay_games:
                logger.info(
                    f"All of your {activated} titles have already been activated on your Ubisoft account."
                )
                return
            ...
            def read_service_response(response):
                ...
                for error in response["errors"]:
                    try:
                        ...
                        logger.debug(
                            f"Reading upstream error message failed with {_e!r}"
                        )
            ...
            try:
                for game in uplay_games:
                    result = self.core.egs.store_claim_uplay_code(
                        ubi_account_id, game.partner_link_id
                    )
                    if "errors" in result:
                        logger.error(
                            f'Claiming "{game.app_title}" failed with: {read_service_response(result)}'
                        )
                    else:
                        logger.debug(
                            f'Successfully claimed code for "{game.partner_link_id}"'
                        )

                result = self.core.egs.store_redeem_uplay_codes(ubi_account_id)
                if "errors" in result:
                    logger.error(
                        f"Redeeming codes (partially) failed with: {read_service_response(result)}"
                    )
            except Exception as e:
            ...
            origin_games = [
                game
                for game in na_games
                if game.third_party_store in ["Origin", "the EA app"]
            ]
            ...
            logger.info(
                "Note: Legendary does not know which of these have already been activated. "
                "Proceeding will result in it attempting to activate all of them."
            )
            logger.info(
                "If Origin asks you to install the title rather than to activate, "
                "it has already been activated, and the dialog can be dismissed."
            )

            if os.name != "nt":
                logger.info(
                    "On non-Windows systems the automatic portion of this process does not work yet, "
            ...
            logger.info(
                "After one title has been processed, hit enter to proceed with the next one."
            )
    ...
    def manage_eos_overlay(self, args):
        ...
        if os.name != "nt":
            if args.app:
                ...
                if sys_platform == "darwin":
                    args.bottle = self.core.lgd.config.get(
                        app_name, "crossover_bottle", fallback=None
                    )

                args.prefix = self.core.lgd.config.get(
                    f"{app_name}.env", "WINEPREFIX", fallback=None
                )
                args.prefix = self.core.lgd.config.get(
                    app_name, "wine_prefix", fallback=args.prefix
                )
            ...
            if sys_platform == "darwin" and args.bottle:
                ...
                logger.error(
                    "Need either --prefix, --bottle, or --app for this command."
                )
                return
        ...
        if args.action == "info":
            ...
            if reg_paths["overlay_path"] and self.core.is_overlay_install(
            ...
            vulkan_overlays = set(reg_paths["vulkan_hkcu"]) | set(
                reg_paths["vulkan_hklm"]
            )
            if vulkan_overlays:
            ...
            if old_path := reg_paths["overlay_path"]:
                if os.path.normpath(old_path) == args.path:
                    ...
                    logger.info(
                        f'Updating overlay registry entries from "{old_path}" to "{args.path}"'
                    )
            ...
            if self.core.is_overlay_installed():
                logger.info(
                    f"To re-enable the overlay, run: legendary eos-overlay enable"
                )
            else:
                logger.info(
                    f'To re-enable the overlay, run: legendary eos-overlay enable --path "{old_path}"'
                )
            ...
            if not args.yes:
                if not get_boolean_choice(
                    "Do you want to uninstall the overlay?", default=False
                ):
                    print("Aborting...")
                    return
            ...
            if os.name != "nt" and not prefix:
                logger.info(
                    "Registry entries in prefixes (if any) have not been removed. "
                ...
                if os.name != "nt":
                    logger.info(
                        f'Registry entries in prefixes other than "{prefix}" were not removed. '
                        f"This shouldn't cause any issues as the overlay will simply fail to load."
                    )
            logger.info("Done.")
                ...
                if os.name == "nt" or prefix:
                    ...
                    if old_path := reg_paths["overlay_path"]:
                        if os.path.normpath(old_path) != install_path:
                            logger.info(
                                f'Updating overlay registry entries from "{old_path}" to "{install_path}"'
                            )
                            remove_registry_entries(prefix)
                        else:
                    ...
                    logger.info(
                        'Overlay has been downloaded. Run "legendary eos-overlay enable -h" to see '
    ...
    def crossover_setup(self, args):
        ...
        if args.crossover_app:
            ...
            if len(apps) > 1:
                ...
                logger.error(
                    f"No CrossOver installs found, see https://legendary.gl/crossover-setup "
        ...
        if args.crossover_bottle:
            ...
            logger.info(
                "It is recommended to set up a bottle specifically for Legendary, see "
                "https://legendary.gl/crossover-setup for setup instructions."
            )
        elif args.download:
            logger.warning(
                "This feature is experimental, and may not work properly. This is not supported by "
                "CodeWeavers/CrossOver, please report issues only on the Legendary GitHub or Discord!"
            )
            if mac_is_crossover_running():
                logger.error(
                    "CrossOver is still running, please quit it before proceeding."
                )
                return
            ...
            if args.disable_version_check:
                logger.warning(
                    "All available bottles are shown, including possibly incompatible ones."
                )
                usable_bottles = available_bottles
            else:
                usable_bottles = [
                    b for b in available_bottles if cx_version in b["cx_versions"]
                ]
            logger.info(
                f"Found {len(usable_bottles)} bottle(s) usable with the selected CrossOver version. "
                f"(Total: {len(available_bottles)})"
            )

            if not usable_bottles:
                logger.info(
                    f"No usable bottles found, see https://legendary.gl/crossover-setup for "
                ...
                for i, bottle in enumerate(usable_bottles, start=1):
                    ...
                    if extra:
                        print(
                            f'  {i:2d}. {bottle["name"]} ({bottle["description"]}) [{", ".join(extra)}]'
                        )
                    else:
                ...
                choice = get_int_choice(
                    f"Select a bottle (CTRL+C to abort)",
                    default_choice,
                    1,
                    len(usable_bottles),
            ...
            if install_candidate:
                ...
                logger.info(
                    f'Preparing to download "{bottle_name}" ({install_candidate["description"]})...'
                )

                if bottle_name in bottles:
                    logger.warning(f"Bottle with the same name already exists!")
                    new_name = input(
                        "Please provide a new name for the bottle [CTRL-C or empty to abort]: "
                    )
                    if not new_name:
                ...
                dlm, ares, path = self.core.prepare_bottle_download(
                    bottle_name,
                ...
                if not args.yes:
                    if not get_boolean_choice(
                        "Do you want to download the selected bottle?"
                    ):
                        print("Aborting...")
                        return
                ...
                try:
                    ...
                    logger.error(
                        f"The following exception occurred while waiting for the downloader: {e!r}. "
                        f"Try restarting the process, if it continues to fail please open an issue on GitHub."
                    )
        ...
        if len(bottles) > 1 and not forced_selection:
            ...
            logger.error(
                "No Bottles found, see https://legendary.gl/crossover-setup for setup instructions."
            )
            return
        ...
        if y_n:
            ...
            self.core.lgd.config.set(
                app_name, "crossover_bottle", args.crossover_bottle
            )
        ...
        if not args.skip_move:
            try:
                ...
                if isinstance(e, OSError) and e.errno == 18:
                    logger.error(
                        f"Moving to a different drive is not supported. Move the folder manually to "
                        f'"{new_path}" and run "legendary move {app_name} "{args.new_path}" --skip-move"'
                    )
                elif isinstance(e, FileExistsError):
                    logger.error(
                        f'The target path already contains a folder called "{game_folder}", '
                        f"please remove or rename it first."
                    )
                else:
                    logger.error(f"Moving failed with unknown error {e!r}.")
                    logger.info(
                        f'Try moving the folder manually to "{new_path}" and running '
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=f'Legendary v{__version__} - "{__codename__}"'
    )
    ...
    parser.add_argument(
        "-v", "--debug", dest="debug", action="store_true", help="Set loglevel to debug"
    )
    parser.add_argument(
        "-y",
        "--yes",
    ...
    activate_parser = subparsers.add_parser(
        "activate", help="Activate games on third party launchers"
    )
    alias_parser = subparsers.add_parser("alias", help="Manage aliases")
    auth_parser = subparsers.add_parser(
        "auth", help="Authenticate with the Epic Games Store"
    )
    clean_saves_parser = subparsers.add_parser("clean-saves", help="Clean cloud saves")
    clean_parser = subparsers.add_parser(
        "cleanup", help="Remove old temporary, metadata, and manifest files"
    )
    cx_parser = subparsers.add_parser(
        "crossover", help="Setup CrossOver for launching games (macOS only)"
    )
    download_saves_parser = subparsers.add_parser(
        "download-saves", help="Download all cloud saves"
    )
    egl_sync_parser = subparsers.add_parser(
        "egl-sync", help="Setup or run Epic Games Launcher sync"
    )
    eos_overlay_parser = subparsers.add_parser(
        "eos-overlay", help="Manage EOS Overlay install"
    )
    ...
    info_parser = subparsers.add_parser(
        "info", help="Prints info about specified app name or manifest"
    )
    install_parser = subparsers.add_parser(
        "install",
        help="Install/download/update/repair a game",
    ...
    list_files_parser = subparsers.add_parser(
        "list-files", help="List files in manifest"
    )
    list_installed_parser = subparsers.add_parser(
        "list-installed", help="List installed games"
    )
    list_saves_parser = subparsers.add_parser(
        "list-saves", help="List available cloud saves"
    )
    move_parser = subparsers.add_parser(
        "move", help="Move specified app name to a new location"
    )
    status_parser = subparsers.add_parser(
        "status", help="Show legendary status information"
    )
    sync_saves_parser = subparsers.add_parser("sync-saves", help="Sync cloud saves")
    uninstall_parser = subparsers.add_parser(
        "uninstall", help="Uninstall (delete) a game"
    )
    ...
    install_parser.add_argument(
        "app_name", help="Name of the app", metavar="<App Name>"
    )
    ...
    list_files_parser.add_argument(
        "app_name", nargs="?", metavar="<App Name>", help="Name of the app (optional)"
    )
    ...
    download_saves_parser.add_argument(
        "app_name",
        nargs="?",
        metavar="<App Name>",
        default="",
    ...
    clean_saves_parser.add_argument(
        "app_name",
        nargs="?",
        metavar="<App Name>",
        default="",
    ...
    sync_saves_parser.add_argument(
        "app_name",
        nargs="?",
        metavar="<App Name>",
        default="",
    ...
    import_parser.add_argument(
        "app_path",
        help="Path where the game is installed",
        metavar="<Installation directory>",
    )
    info_parser.add_argument(
        "app_name_or_manifest",
        help="App name or manifest path/URI",
        metavar="<App Name/Manifest URI>",
    )

    alias_parser.add_argument(
        "action",
        help="Action: Add, rename, remove, or list alias(es)",
        choices=["add", "rename", "remove", "list"],
        metavar="<add|rename|remove|list>",
    )
    alias_parser.add_argument(
        "app_or_alias",
        help='App name when using "add" or "list" action, '
        'existing alias when using "rename" or "remove" action',
        metavar="<App name/Old alias>",
        nargs="?",
    )
    alias_parser.add_argument(
        "alias",
        help='New alias when using "add" action',
        metavar="<New alias>",
        nargs="?",
    )
    ...
    move_parser.add_argument(
        "new_path", metavar="<New Base Path>", help="Directory to move game folder to"
    )

    # Flags
    auth_parser.add_argument(
        "--import",
        dest="import_egs_auth",
        action="store_true",
    ...
    if os.name != "nt":
        ...
        launch_parser.add_argument(
            "--wine-prefix", help=argparse.SUPPRESS, dest="wine_pfx"
        )
        launch_parser.add_argument(
            "--no-wine",
            dest="no_wine",
    ...
    if sys_platform == "darwin":
        ...
        launch_parser.add_argument(
            "--crossover", dest="crossover", action="store_true", help=argparse.SUPPRESS
        )
        launch_parser.add_argument(
            "--crossover-app",
            dest="crossover_app",
    ...
    list_parser.add_argument(
        "--csv", dest="csv", action="store_true", help="List games in CSV format"
    )
    list_parser.add_argument(
        "--tsv", dest="tsv", action="store_true", help="List games in TSV format"
    )
    list_parser.add_argument(
        "--json", dest="json", action="store_true", help="List games in JSON format"
    )
    list_parser.add_argument(
        "--force-refresh",
        dest="force_refresh",
    ...
    list_installed_parser.add_argument(
        "--check-updates",
        dest="check_updates",
        action="store_true",
    ...
    list_files_parser.add_argument(
        "--csv", dest="csv", action="store_true", help="Output in CSV format"
    )
    list_files_parser.add_argument(
        "--tsv", dest="tsv", action="store_true", help="Output in TSV format"
    )
    list_files_parser.add_argument(
        "--json", dest="json", action="store_true", help="Output in JSON format"
    )
    list_files_parser.add_argument(
        "--hashlist",
        dest="hashlist",
    ...
    sync_saves_parser.add_argument(
        "--accept-path", dest="accept_path", action="store_true", help=argparse.SUPPRESS
    )

    clean_saves_parser.add_argument(
        "--delete-incomplete",
        dest="delete_incomplete",
        action="store_true",
    ...
    egl_sync_parser.add_argument(
        "--egl-manifest-path",
        dest="egl_manifest_path",
        action="store",
    ...
    status_parser.add_argument(
        "--json", dest="json", action="store_true", help="Show status in JSON format"
    )
    ...
    eos_overlay_parser.add_argument(
        "action",
        help="Action: install, remove, enable, disable, "
    ...
    if os.name != "nt":
        ...
        if sys_platform == "darwin":
            ...
            eos_overlay_parser.add_argument(
                "--bottle", dest="bottle", action="store", help=argparse.SUPPRESS
            )
    else:
        eos_overlay_parser.add_argument(
            "--prefix", dest="prefix", action="store", help=argparse.SUPPRESS
        )
        eos_overlay_parser.add_argument(
            "--bottle", dest="bottle", action="store", help=argparse.SUPPRESS
        )
        eos_overlay_parser.add_argument(
            "--app", dest="app", action="store", help=argparse.SUPPRESS
        )
    ...
    if not args.subparser_name or args.full_help:
        ...
        if args.full_help:
            ...
            if double_clicked():
                ...
                print(
                    "Follow https://github.com/derrod/legendary/wiki/Setup-Instructions to set it up properly"
                )
    ...
    if hasattr(args, "platform"):
        if not args.platform:
            ...
            args.platform = cli.core.lgd.config.get(
                "Legendary", "default_platform", fallback=os_default
            )
        elif args.platform not in ("Win32", "Windows", "Mac"):
            logger.warning(
                f'Platform "{args.platform}" may be invalid. Valid ones are: Windows, Win32, Mac.'
            )
    ...
    if (
        not disable_update_message
        and cli.core.update_available
        and cli.core.update_notice_enabled()
    ):
        if update_info := cli.core.get_update_info():
            ...
            if not is_windows_mac_or_pyi():
                print(
                    "If you installed legendary via a package manager it may "
