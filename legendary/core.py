...
class LegendaryCore:
    ...
    def __init__(self, override_config=None, timeout=10.0):
        ...
        if os.name != "nt":
            self.egl.programdata_path = self.lgd.config.get(
                "Legendary", "egl_programdata", fallback=None
            )
            if self.egl.programdata_path and not os.path.exists(
                self.egl.programdata_path
            ):
                self.log.error(
                    f'Config EGL path ("{self.egl.programdata_path}") is invalid! Disabling sync...'
                )
        ...
        if locale := self.lgd.config.get(
            "Legendary",
            "locale",
            fallback=getdefaultlocale(("LANG", "LANGUAGE", "LC_ALL", "LC_CTYPE"))[0],
        ):
            try:
                self.language_code, self.country_code = locale.split(
                    "-" if "-" in locale else "_"
                )
                self.log.debug(
                    f"Set locale to {self.language_code}-{self.country_code}"
                )
                # adjust egs api language as well
                self.egs.language_code, self.egs.country_code = (
                    self.language_code,
                    self.country_code,
                )
            except Exception as e:
                self.log.warning(
                    f"Getting locale failed: {e!r}, falling back to using en-US."
                )
    ...
    def update_check_enabled(self):
        return not self.lgd.config.getboolean(
            "Legendary", "disable_update_check", fallback=False
        )
    ...
    def update_notice_enabled(self):
        if self.force_show_update:
            return True
        return not self.lgd.config.getboolean(
            "Legendary", "disable_update_notice", fallback=not is_windows_mac_or_pyi()
        )
    ...
    def check_for_updates(self, force=False):
        ...
        if (
            force
            or not version_info
            or (datetime.now().timestamp() - cached["last_update"]) > 24 * 3600
        ):
    ...
    def apply_lgd_config(self, version_info=None):
        ...
        if "egl_config" in version_info:
            ...
            self._egl_version = version_info["egl_config"].get(
                "version", self._egl_version
            )
    ...
    def get_sdl_data(self, app_name, platform="Windows"):
        ...
        version_info = self.lgd.get_cached_version()["data"]
        latest = (
            version_info.get("game_overrides", {}).get("sdl_config", {}).get(app_name)
        )
        if (not cached and latest) or (
            cached and latest and latest > cached["version"]
        ):
            try:
                sdl_data = self.lgdapi.get_sdl_config(app_name)
                self.log.debug(
                    f'Downloaded SDL data for "{app_name}", version: {latest}'
                )
    ...
    def update_aliases(self, force=False):
        _aliases_enabled = not self.lgd.config.getboolean(
            "Legendary", "disable_auto_aliasing", fallback=False
        )
            ...
            assets.update(
                {
                    platform: [
                        ...
                        for a in self.egs.get_game_assets(platform=platform)
                    ]
                }
            )
    ...
    def get_game_list(self, update_assets=True, platform="Windows") -> List[Game]:
        return self.get_game_and_dlc_list(
            update_assets=update_assets, platform=platform
        )[0]

    def get_game_and_dlc_list(
        self, update_assets=True, platform="Windows", force_refresh=False, skip_ue=True
    ) -> (List[Game], Dict[str, List[Game]]):
        _ret = []
        ...
        for app_name, app_assets in sorted(assets.items()):
            ...
            if game:
                asset_updated = any(
                    game.app_version(_p) != app_assets[_p].build_version
                    for _p in app_assets.keys()
                )
                games[app_name] = game

            if update_assets and (
                not game or force_refresh or (game and asset_updated)
            ):
        ...
        for app_name, app_assets in sorted(assets.items()):
            ...
            if not game or app_name in still_needs_update:
                if use_threads:
                    self.log.warning(
                        f"Fetching metadata for {app_name} failed, retrying"
                    )
                _ga = next(iter(app_assets.values()))
    ...
    def get_non_asset_library_items(
        self, force_refresh=False, skip_ue=True
    ) -> (List[Game], Dict[str, List[Game]]):
        """
    ...
    def get_installed_game(self, app_name, skip_sync=False) -> InstalledGame:
        ...
        if (
            not skip_sync
            and igame
            and self.egl_sync_enabled
            and igame.egl_guid
            and not igame.is_dlc
        ):
    ...
    def get_app_environment(
        self, app_name, wine_pfx=None, cx_bottle=None, disable_wine=False
    ) -> dict:
        ...
        if sys_platform == "darwin":
            if cx_bottle:
                ...
                "CX_BOTTLE" not in os.environ
            ):  # Only fall back to config is not already set in env
                cx_bottle = self.lgd.config.get(
                    "default", "crossover_bottle", fallback="Legendary"
                )
                cx_bottle = self.lgd.config.get(
                    app_name, "crossover_bottle", fallback=cx_bottle
                )
                if cx_bottle:
                    env["CX_BOTTLE"] = cx_bottle
            else:
    ...
    def get_app_launch_command(
        ...
        _cmd = []
        if wrapper or (
            wrapper := self.lgd.config.get(
                app_name,
                "wrapper",
                fallback=self.lgd.config.get("default", "wrapper", fallback=None),
            )
        ):
        ...
        if os.name != "nt" and not disable_wine:
            if sys_platform == "darwin" and not crossover_app and not wine_binary:
                crossover_app = self.lgd.config.get(
                    "default", "crossover_app", fallback=None
                )
                crossover_app = self.lgd.config.get(
                    app_name, "crossover_app", fallback=crossover_app
                )
                # on Mac we use CrossOver by default if we can
                if not crossover_app and not self.lgd.config.getboolean(
                    "Legendary", "disable_auto_crossover", fallback=False
                ):
                    cx_apps = mac_find_crossover_apps()
                    cx_version, crossover_app = cx_apps[0]
                    self.log.debug(
                        f"Default CrossOver found: {crossover_app} ({cx_version})"
                    )
            ...
            if sys_platform == "darwin" and crossover_app:
                ...
                if os.path.exists(wine_path):
                    ...
                    self.log.warning(
                        f"Specified CrossOver app not valid (no wine binary): {crossover_app}"
                    )

            if not wine_binary:
                # check if there's a default override
                wine_binary = self.lgd.config.get(
                    "default", "wine_executable", fallback="wine"
                )
                # check if there's a game specific override
                wine_binary = self.lgd.config.get(
                    app_name, "wine_executable", fallback=wine_binary
                )
    ...
    def get_pre_launch_command(self, app_name=""):
        if app_name:
            pre_launch_command = self.lgd.config.get(
                app_name, "pre_launch_command", fallback=None
            )
            pre_launch_wait = self.lgd.config.getboolean(
                app_name, "pre_launch_wait", fallback=False
            )
        ...
        pre_launch_command = self.lgd.config.get(
            "default", "pre_launch_command", fallback=None
        )
        pre_launch_wait = self.lgd.config.getboolean(
            "default", "pre_launch_wait", fallback=False
        )
        return pre_launch_command, pre_launch_wait
    ...
    def get_launch_parameters(
        ...
        if executable_override or (
            executable_override := self.lgd.config.get(
                app_name, "override_exe", fallback=None
            )
        ):
        ...
        params = LaunchParameters(
            ...
            game_directory=install.install_path,
            working_directory=working_dir,
            launch_command=self.get_app_launch_command(
                app_name=app_name,
                wrapper=wrapper,
                wine_binary=wine_bin,
                crossover_app=crossover_app,
                disable_wine=disable_wine,
            ),
            environment=self.get_app_environment(
                app_name=app_name,
                wine_pfx=wine_pfx,
                cx_bottle=crossover_bottle,
                disable_wine=disable_wine,
            ),
        )
        ...
        if install.launch_parameters:
            try:
                params.game_parameters.extend(
                    shlex.split(install.launch_parameters, posix=False)
                )
            except ValueError as e:
                self.log.warning(
                    f"Parsing predefined launch parameters failed with: {e!r}, "
        ...
        if meta_args := game.additional_command_line:
            try:
                params.game_parameters.extend(
                    shlex.split(meta_args.strip(), posix=False)
                )
            except ValueError as e:
                self.log.warning(
                    f"Parsing metadata launch parameters failed with: {e!r}, "
                    f"input: {install.launch_parameters}"
                )
        ...
        if install.requires_ot and not offline:
            ...
            ovt_path = os.path.join(
                self.lgd.get_tmp_path(), f"{game.namespace}{game.catalog_item_id}.ovt"
            )
    ...
    def get_save_path(self, app_name, platform="Windows"):
        ...
        if sys_platform == "win32":
            ...
            if sys_platform == "darwin":
                cx_bottle = self.lgd.config.get(
                    f"{app_name}.env", "CX_BOTTLE", fallback=None
                )
                cx_bottle = self.lgd.config.get(
                    app_name, "crossover_bottle", fallback=cx_bottle
                )
            ...
            if not wine_pfx and sys_platform == "darwin":
                cx_bottle = self.lgd.config.get(
                    "default.env", "CX_BOTTLE", fallback=None
                )
                cx_bottle = self.lgd.config.get(
                    "default", "crossover_bottle", fallback=cx_bottle
                )
                if cx_bottle and mac_is_valid_bottle(cx_bottle):
                    wine_pfx = mac_get_bottle_path(cx_bottle)
            ...
            if not wine_pfx:
                wine_pfx = self.lgd.config.get(
                    "default.env", "WINEPREFIX", fallback=None
                )
                wine_pfx = self.lgd.config.get(
                    "default", "wine_prefix", fallback=wine_pfx
                )
            ...
            if os.path.isdir(wine_pfx):
                ...
                path_vars["{usersavedgames}"] = wine_folders[
                ...
                if "roaming" not in wine_folders["AppData"].lower():
                    path_vars["{locallow}"] = wine_folders[
                    ...
                    if "locallow" in save_path_lower:
                        save_path = (
                            "{locallow}/"
                            + save_path[save_path_lower.index("locallow/") + 9 :]
                        )
                    elif "roaming" in save_path_lower:
                        save_path = (
                            "{roaming}/"
                            + save_path[save_path_lower.index("roaming/") + 8 :]
                        )

                    self.log.debug(
                        f'Legacy AppData paths detected, adjusted save game folder to "{save_path}"'
                    )
    ...
    def check_savegame_state(
        self, path: str, save: SaveGameFile
    ) -> (SaveGameStatus, (datetime, datetime)):
        latest = 0
        ...
        dt_remote = datetime.strptime(
            save.manifest_name, "%Y.%m.%d-%H.%M.%S.manifest"
        ).replace(tzinfo=timezone.utc)
        if not latest:
            return SaveGameStatus.REMOTE_NEWER, (None, dt_remote)

        self.log.debug(
            f"Local save date: {str(dt_local)}, Remote save date: {str(dt_remote)}"
        )
    ...
    def upload_save(
        ...
        if not disable_filtering:
            ...
            if (
                _include := custom_attr.get("CloudIncludeList", {}).get("value", None)
            ) is not None:
                include_f = _include.split(",")
            if (
                _exclude := custom_attr.get("CloudExcludeList", {}).get("value", None)
            ) is not None:
        ...
        if not files:
            self.log.info(
                'No files to upload. If you believe this is incorrect run command with "--disable-filters"'
            )
            return
    ...
    def download_saves(
        self, app_name="", manifest_name="", save_dir="", clean_dir=False
    ):
        ...
        for fname, f in files.items():
            ...
            if not save_dir:
                _save_dir = os.path.join(
                    save_path, f'{f_parts[2]}/{f_parts[4].rpartition(".")[0]}'
                )
            ...
            if app_name and clean_dir:
                ...
                if (
                    _include := custom_attr.get("CloudIncludeList", {}).get(
                        "value", None
                    )
                ) is not None:
                    include_f = _include.split(",")
                if (
                    _exclude := custom_attr.get("CloudExcludeList", {}).get(
                        "value", None
                    )
                ) is not None:
                    exclude_f = _exclude.split(",")
            ...
            for chunk in m.chunk_data_list.elements:
                ...
                if cpath not in files:
                    self.log.warning(
                        f"Chunk {cpath} not in file list, save data may be incomplete!"
                    )
                    continue
            ...
            if not chunks:
                if manifest_name:
                    self.log.fatal(
                        f"No chunks were available, aborting. Try running "
                        f'"legendary clean-saves {app_name}" and try again.'
                    )
                    return
                else:
                    self.log.error(
                        f'No chunks were available, skipping. You can run "legendary clean-saves" '
                ...
                with open(fpath, "wb") as fh:
                    for cp in fm.chunk_parts:
                        if cp.guid_num not in chunks:
                            self.log.error(
                                f"Chunk part for {fname} is missing, file may be corrupted!"
                            )
                        else:
                            fh.write(
                                chunks[cp.guid_num][cp.offset : cp.offset + cp.size]
                            )

                # set modified time to savegame creation timestamp
                m_date = datetime.strptime(f_parts[4], "%Y.%m.%d-%H.%M.%S.manifest")
                m_date = m_date.replace(tzinfo=timezone.utc).astimezone(
                    self.local_timezone
                )
    ...
    def clean_saves(self, app_name="", delete_incomplete=False):
        ...
        for fname, f in files.items():
            ...
            if r.status_code == 404:
                ...
                self.log.warning(
                    f"Download failed, status code: {r.status_code}. Skipping..."
                )
            ...
            if (
                0 < missing_chunks < total_chunks and delete_incomplete
            ) or missing_chunks == total_chunks:
                self.log.error("Chunk(s) missing, marking manifest for deletion.")
                deletion_list.append(fname)
                continue
            elif 0 < missing_chunks < total_chunks:
                self.log.error(
                    f'Some chunk(s) missing, optionally run "legendary download-saves" to obtain a backup '
    ...
    def get_cdn_urls(self, game, platform="Windows"):
        m_api_r = self.egs.get_game_manifest(
            game.namespace, game.catalog_item_id, game.app_name, platform
        )
    ...
    def get_cdn_manifest(self, game, platform="Windows", disable_https=False):
        ...
        if disable_https:
            manifest_urls = [
                url.replace("https://", "http://") for url in manifest_urls
            ]
        ...
        for url in manifest_urls:
            ...
            try:
                ...
                self.log.warning(
                    f'Unable to download manifest from "{urlparse(url).netloc}" '
            ...
            raise ValueError(
                f"Unable to get manifest from any CDN URL, last result: {r.status_code} ({r.reason})"
            )
    ...
    def get_delta_manifest(self, base_url, old_build_id, new_build_id):
        ...
        r = self.egs.unauth_session.get(
            f"{base_url}/Deltas/{new_build_id}/{old_build_id}.delta"
        )
    ...
    def prepare_download(
        ...
        disable_https = disable_https or self.lgd.config.getboolean(
            "Legendary", "disable_https", fallback=False
        )
        ...
        disable_delta = disable_delta or (
            (override_old_manifest or override_manifest) and not override_delta_manifest
        )
        ...
        if old_manifest and new_manifest and not disable_delta:
            if override_delta_manifest:
                self.log.info(
                    f'Overriding delta manifest with "{override_delta_manifest}"'
                )
                delta_manifest_data, _ = self.get_uri_manifest(override_delta_manifest)
            else:
                delta_manifest_data = self.get_delta_manifest(
                    base_urls[0], old_manifest.meta.build_id, new_manifest.meta.build_id
                )
            if delta_manifest_data:
                delta_manifest = self.load_manifest(delta_manifest_data)
                self.log.info(
                    f"Using optimized delta manifest to upgrade from build "
        ...
        if igame := self.get_installed_game(
            base_game.app_name if base_game else game.app_name
        ):
            ...
            if not game_folder:
                if game.is_dlc:
                    game_folder = (
                        base_game.metadata.get("customAttributes", {})
                        .get("FolderName", {})
                        .get("value", base_game.app_name)
                    )
                else:
                    game_folder = (
                        game.metadata.get("customAttributes", {})
            ...
            if not base_path:
                ...
                if platform == "Mac":
                    ...
                    if all(
                        ".app" in fm.filename.partition("/")[0].lower()
            ...
            install_path = os.path.normpath(
                os.path.join(base_path, game_folder.strip())
            )
        ...
        if override_base_url:
        ...
        elif preferred_cdn or (
            preferred_cdn := self.lgd.config.get(
                "Legendary", "preferred_cdn", fallback=None
            )
        ):
            for url in base_urls:
                ...
                self.log.warning(
                    f'Preferred CDN "{preferred_cdn}" unavailable, using default selection.'
                )
        ...
        if dl_optimizations or is_opt_enabled(
            game.app_name, new_manifest.meta.build_version
        ):
        ...
        if new_manifest.meta.prereq_ids:
            prereq = dict(
                ids=new_manifest.meta.prereq_ids,
                name=new_manifest.meta.prereq_name,
                path=new_manifest.meta.prereq_path,
                args=new_manifest.meta.prereq_args,
            )

        uninstaller = None
        if new_manifest.meta.uninstall_action_path:
            uninstaller = dict(
                path=new_manifest.meta.uninstall_action_path,
        ...
        if platform != "Mac" and (
            exe_override := get_exe_override(app_name=game.app_name)
        ):
            exe_override_l = exe_override.lower()
            # make sure that override exe even exists
            if any(
                fm.filename.lower() == exe_override_l
                for fm in new_manifest.file_manifest_list.elements
            ):
                self.log.info(
                    f'Launch exe will be changed from "{executable}" to "{exe_override}" for compatibility'
                )
        ...
        igame = InstalledGame(
            ...
            can_run_offline=offline == "true",
            requires_ot=ot == "true",
            is_dlc=base_game is not None,
            install_size=anlres.install_size,
            egl_guid=egl_guid,
            install_tags=file_install_tag,
            platform=platform,
    ...
    @staticmethod
    def check_installation_conditions(
            ...
            for f in analysis.manifest_comparison.added:
                ...
                if "easyanticheat" in flower:
                    results.warnings.add(
                        "(Linux) This game uses EasyAntiCheat and may not run on linux"
                    )
                elif "beclient" in flower:
                    results.warnings.add(
                        "(Linux) This game uses BattlEye and may not run on linux"
                    )
                elif "equ8.dll" in flower:
                    results.warnings.add(
                        "(Linux) This game is using EQU8 anticheat and may not run on linux"
                    )
                elif flower == "fna.dll" or flower == "xna.dll":
                    results.warnings.add(
                        "(Linux) This game is using XNA/FNA and may not run in WINE"
                    )

        if install.requires_ot:
            results.warnings.add(
                "This game requires an ownership verification token and likely uses Denuvo DRM."
            )
        if not install.can_run_offline:
            results.warnings.add(
                "This game is not marked for offline use (may still work)."
            )
        ...
        if os.path.exists(base_path):
            ...
            if free < min_disk_space:
                ...
                if ignore_space_req:
                    results.warnings.add(
                        f"Potentially not enough available disk space! "
                        f"{free_gib:.02f} GiB < {required_gib:.02f} GiB"
                    )
                else:
                    results.failures.add(
                        f"Not enough available disk space! "
                        f"{free_gib:.02f} GiB < {required_gib:.02f} GiB"
                    )
        else:
            results.failures.add(
                f'Install path "{base_path}" does not exist, make sure all necessary mounts are '
        ...
        executables = [
            f
            for f in analysis.manifest_comparison.added
            if f.lower().endswith(".exe") and not f.startswith("Installer/")
        ]
        if (
            not updating
            and not any("uplay" not in e.lower() for e in executables)
            and any("uplay" in e.lower() for e in executables)
        ):
            uplay_required = True
            results.failures.add(
                "This game requires installation via Uplay and does not ship executable game files."
            )
        ...
        if game.partner_link_type and game.partner_link_type != "ubisoft":
            results.warnings.add(
                f'This game requires linking to "{game.partner_link_type}", '
        ...
        if uplay_required or game.partner_link_type == "ubisoft":
            if os.name == "nt":
                results.warnings.add(
                    "This game requires installation of Uplay/Ubisoft Connect, direct "
        ...
        if "2klauncher" in install.executable.lower():
            results.warnings.add(
                "This game uses the 2K Launcher which is does not work with Legendary. "
                "Consult the Legendary or EpicLinux Wikis for workarounds (e.g. exe override)."
            )
            # suggest alternative EXEs
            alts = []
            all_files = (
                analysis.manifest_comparison.added
            ...
            results.warnings.add(
                "You may want to consider trying one of the following executables "
        ...
        eos_installer = next(
            (
                f
                for f in analysis.manifest_comparison.added
                if "epiconlineservicesinstaller" in f.lower()
            ),
            None,
        )
        has_bootstrapper = any(
            "eosbootstrapper" in f.lower() for f in analysis.manifest_comparison.added
        )

        if eos_installer:
            results.warnings.add(
                "This game ships the Epic Online Services Windows service, "
            ...
            results.warnings.add(
                "This game ships the Epic Online Services bootstrapper. "
    ...
    def get_default_install_dir(self, platform="Windows"):
        if platform == "Mac":
            return os.path.expanduser(
                self.lgd.config.get(
                    "Legendary", "mac_install_dir", fallback="~/Applications"
                )
            )
        else:
            return os.path.expanduser(
                self.lgd.config.get("Legendary", "install_dir", fallback="~/Games")
            )

    def install_game(self, installed_game: InstalledGame) -> dict:
        if (
            self.egl_sync_enabled
            and not installed_game.is_dlc
            and installed_game.platform.startswith("Win")
        ):
    ...
    def uninstall_game(
        ...
        if delete_files:
            ...
            if (
                installed_game.platform == "Mac"
                and installed_game.install_path.endswith("/Applications")
            ):
                delete_root_directory = False

            try:
                manifest = self.load_manifest(
                    self.get_installed_manifest(installed_game.app_name)[0]
                )
                filelist = [
                    fm.filename
                    for fm in manifest.file_manifest_list.elements
                    if not fm.install_tags
                    or any(t in installed_game.install_tags for t in fm.install_tags)
                ]
                if not delete_filelist(
                    installed_game.install_path, filelist, delete_root_directory
                ):
                    self.log.error(
                        f'Deleting "{installed_game.install_path}" failed, please remove manually.'
                    )
            except Exception as e:
                self.log.error(
                    f"Deleting failed with {e!r}, please remove {installed_game.install_path} manually."
                )
    ...
    def uninstall_tag(self, installed_game: InstalledGame):
        ...
        filelist = [
            ...
            for fm in manifest.file_manifest_list.elements
            if not any(
                ((fit in fm.install_tags) or (not fit and not fm.install_tags))
                for fit in tags
            )
        ...
        if not delete_filelist(installed_game.install_path, filelist):
            self.log.warning(
                f"Deleting some deselected files failed, please check/remove manually."
            )
    ...
    def import_game(
        ...
        if not game.is_dlc and os.path.exists(os.path.join(app_path, ".egstore")):
            ...
            if not egl_guid:
                for f in os.listdir(os.path.join(app_path, ".egstore")):
                    ...
                    mancpn = json.load(
                        open(os.path.join(app_path, ".egstore", f), "rb")
                    )
            ...
            if mf and os.path.exists(os.path.join(app_path, ".egstore", mf)):
                manifest_data = open(
                    os.path.join(app_path, ".egstore", mf), "rb"
                ).read()
            else:
                self.log.warning(
                    ".egstore folder exists but manifest file is missing, continuing as regular import..."
                )
            ...
            if mf and not os.path.exists(os.path.join(app_path, ".egstore", "bps")):
                ...
                if not needs_verify:
                    self.log.debug(
                        f"No in-progress installation found, assuming complete..."
                    )
        ...
        install_size = sum(
            fm.file_size for fm in new_manifest.file_manifest_list.elements
        )
    ...
    def egl_get_importable(self):
        return [
            g
            for g in self.egl.get_manifests()
            if not self.is_installed(g.app_name)
    ...
    def egl_get_exportable(self):
        ...
        return [
            g
            for g in self.get_installed_list()
            if g.app_name not in self.egl.manifests and g.platform.startswith("Win")
        ]

    def egl_import(self, app_name):
        if not self.asset_valid(app_name):
            raise ValueError(
                f"To-be-imported game {app_name} not in game asset database!"
            )
        ...
        try:
            ...
            self.log.fatal(
                f"EGL Manifest for {app_name} could not be loaded, not importing!"
            )
            return
        ...
        if os.name != "nt" and not lgd_igame.install_path.startswith("/"):
            ...
            mapped_path = os.path.realpath(
                os.path.join(wine_pfx, "dosdevices", drive_letter)
            )
            if "dosdevices" in mapped_path:
                self.log.error(
                    f'Unable to resolve path for mapped drive "{drive_letter}" '
            ...
            self.log.info(
                f'Adjusted game install path from "{lgd_igame.install_path}" to "{new_path}"'
            )
            lgd_igame.install_path = new_path

        # check if manifest exists
        manifest_filename = os.path.join(
            lgd_igame.install_path, ".egstore", f"{lgd_igame.egl_guid}.manifest"
        )
        if not os.path.exists(manifest_filename):
            self.log.warning(
                f'Game Manifest "{manifest_filename}" not found, cannot import!'
            )
            return
        ...
        if lgd_igame.install_tags:
            self.lgd.config.set(
                app_name, "install_tags", ",".join(lgd_igame.install_tags)
            )
    ...
    def egl_export(self, app_name):
        ...
        with open(
            os.path.join(
                egstore_folder,
                f"{egl_game.installation_guid}.manifest",
            ),
            "wb",
        ) as mf:
        ...
        with open(
            os.path.join(
                egstore_folder,
                f"{egl_game.installation_guid}.mancpn",
            ),
            "w",
        ) as mcpnf:
    ...
    def egl_uninstall(self, igame: InstalledGame, delete_files=True):
        try:
            ...
            self.log.warning(
                f"EGL manifest was already deleted, in case you uninstalled the Epic Games Launcher"
    ...
    def egl_restore_or_uninstall(self, igame):
        ...
        if not os.path.exists(
            os.path.join(igame.install_path, igame.executable.lstrip("/"))
        ):
            self.log.warning(
                "Synced game's files no longer exists, assuming it has been uninstalled."
            )
            igame.egl_guid = ""
            return self.uninstall_game(igame, delete_files=False)
        else:
            self.log.info(
                "Game files exist, assuming game is still installed, re-exporting to EGL..."
            )
    ...
    def egl_sync(self, app_name=""):
        ...
        if app_name:
            ...
            if app_name not in self.egl.manifests:
                self.log.info(
                    f'Synced app "{app_name}" is no longer in the EGL manifest list.'
                )
                ...
                if (egl_igame.app_version_string != lgd_igame.version) or (
                    ...
                    self.log.info(
                        f'App "{egl_igame.app_name}" has been updated from EGL, syncing...'
                    )
            ...
            for lgd_igame in self._get_installed_list():
                ...
                self.log.info(
                    f'Synced app "{lgd_igame.app_name}" is no longer in the EGL manifest list.'
                )
    ...
    def check_for_overlay_updates(self):
        ...
        if (
            ...
            manifest_response = self.egs.get_game_manifest(
                EOSOverlayApp.namespace,
        ...
        self.overlay_update_available = (
            version_info["buildVersion"] != installed.version
        )
    ...
    def search_overlay_installs(self, prefix=None):
        ...
        if os.name == "nt":
            ...
            locations.append(
                os.path.expandvars(
                    r"%programfiles(x86)%\Epic Games\Launcher\Portal\Extras\Overlay"
                )
            )
            # EOSH path
            locations.append(
                os.path.expandvars(
                    f"%programfiles(x86)%\\Epic Games\\Epic Online Services"
    ...
    def prepare_overlay_install(self, path=None):
        ...
        if free < install_size:
            raise ValueError(
                f"Not enough space to install overlay: {free / 1024 / 1024:.02f} "
                f"MiB < {install_size / 1024 / 1024:.02f} MiB"
            )

        igame = InstalledGame(
            app_name=EOSOverlayApp.app_name,
            title=EOSOverlayApp.app_title,
    ...
    def prepare_bottle_download(self, bottle_name, manifest_url, base_url=None):
        ...
        if free < install_size:
            raise ValueError(
                f"Not enough space to setup bottle: {free / 1024 / 1024:.02f} "
                f"MiB < {install_size / 1024 / 1024:.02f} MiB"
            )
    ...
    def finish_bottle_setup(self, bottle_name):
        ...
        symlinks = [
            ...
            ("dosdevices/z:", "/"),
            (
                "drive_c/users/crossover/Desktop/My Mac Desktop",
            ...
            (
                "drive_c/users/crossover/Templates",
                os.path.join(path, "dosdevices/c:/users/crossover/My Documents"),
            ),
        ]
